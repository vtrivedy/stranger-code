"""Textual UI application for Stranger Code - The Upside Down of AI."""
...
from stranger_code.clipboard import copy_selection_to_clipboard
from stranger_code.textual_adapter import TextualUIAdapter, execute_task_textual
from stranger_code.widgets.approval import ApprovalMenu
from stranger_code.widgets.chat_input import ChatInput
from stranger_code.widgets.loading import LoadingWidget
...
from stranger_code.widgets.christmas import ChristmasLights
from stranger_code.widgets.splash import SplashComplete, SplashOverlay
...
class DeepAgentsApp(App):
    """Main Textual application for Stranger Code - Enter the Upside Down."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize the Stranger Code application.
        ...
        Args:
            ...
            no_splash: Skip the Stranger Things intro sequence
        ...
        # Christmas mode (Joyce's lights!)
        self._christmas_mode = False
        self._christmas_lights: ChristmasLights | None = None

    def compose(self) -> ComposeResult:
        """Compose the application layout."""
        # Christmas lights at the very top (hidden by default)
        yield ChristmasLights(light_count=40, id="christmas-lights-top")
        ...
        # Christmas lights above status bar (hidden by default)
        yield ChristmasLights(light_count=40, id="christmas-lights-bottom")

        # Status bar at bottom
        yield StatusBar(cwd=self._cwd, id="status-bar")

        # Splash screen overlay (if not skipped)
        if not self._no_splash:
            yield SplashOverlay(id="splash-overlay")
    ...
    async def on_mount(self) -> None:
        ...
        # Focus handling
        if self._no_splash:
            # Focus the input if splash is skipped
            self._chat_input.focus_input()
        else:
            # Focus the splash screen so it receives key events
            try:
                from stranger_code.widgets.splash import SplashScreen
    ...
    def on_splash_complete(self, event: SplashComplete) -> None:
        """Handle splash screen completion - focus input and mark complete."""
        self._splash_complete = True
        if self._chat_input:
            self._chat_input.focus_input()

    def on_key(self, event) -> None:
        """Handle key events at app level - needed for splash screen Enter."""
        # If splash is showing and not complete, handle Enter for portal transition
        if not self._splash_complete and not self._no_splash:
            if event.key == "enter":
                try:
                    from stranger_code.widgets.splash import SplashScreen
                    splash = self.query_one(SplashScreen)
                    # Trigger portal transition if in prompt phase
                    if splash.phase == "prompt":
                        splash._start_portal_transition()
                        event.stop()
                except NoMatches:
                    pass
    ...
    async def _handle_command(self, command: str) -> None:
        ...
        if cmd in ("/quit", "/exit", "/q"):
            ...
            await self._mount_message(
                SystemMessage("Commands: /quit, /clear, /tokens, /threads, /christmas, /help")
            )
        elif cmd == "/clear":
            ...
            await self._toggle_christmas_mode()
        else:
            await self._mount_message(UserMessage(command))
    ...
    async def _toggle_christmas_mode(self) -> None:
        """Toggle Joyce's Christmas lights mode."""
        self._christmas_mode = not self._christmas_mode

        try:
            lights_top = self.query_one("#christmas-lights-top", ChristmasLights)
            lights_bottom = self.query_one("#christmas-lights-bottom", ChristmasLights)

            lights_top.active = self._christmas_mode
            lights_bottom.active = self._christmas_mode

            if self._christmas_mode:
                await self._mount_message(
                    SystemMessage("Joyce's Christmas lights activated! The Upside Down is communicating...")
                )
            else:
                await self._mount_message(SystemMessage("Christmas lights deactivated."))
        except NoMatches:
            pass
...
async def run_textual_app(
    *,
    ...
    """Run the Stranger Code Textual application.
